...
int main()
{
    std::string input {"word, function"};
    inputOption<int> processMode {optionId::word, NO_VALUE};
    inputOption<int> funcMode {optionId::fill, NO_VALUE};
    inputOption<float> additionalProcessMode {optionId::percent, 90.0f};
    std::vector<std::unique_ptr<Element>> inputArr {};
    elementProcessMode(input, inputArr, processMode);
    functionMode(inputArr, funcMode, additionalProcessMode);
    for (auto& p : inputArr)
    {
        std::cout << *p << '\n';
    }
